...
# if only one seriesuid is copied, a list of filenames is returned and can be used
list = c.copy_dicom_files_to_dest(seriesuid='1.2...8024597.12' ,destination='out')
for fn in list:
  c.change_tag(fn, tag='PatientBirthDate', new_value='19000101')

# also works with a list of seriesuids, but then no list of filenames is returned
...
# with tuple instead of name of tag
c.change_tag(filename=new_filename, tag=('0x3006', '0x0002'), new_value='StructureSet')
...
- Added option seriesuid to method **query_dicom()**
- Method : **copy_dicom_files()** now returns a list of all copied files
- Added method **calculate_robust_hash(seriesuid)**, to calculate a more robust hash for RTSTRUCT
- bugfix : **change_roinames** can now handle empty contours
